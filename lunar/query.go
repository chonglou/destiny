...
//Load load data from file, named by tmp/{year}.txt
func (p *Query) Load(dir string) error {
...
func (p *Query) current(day string, last *Date) error {
	//mth := last.LMonth
	switch day {
	case "初二":
		last.LDay = 2
...
func (p *Query) read(year int, last *Date) error {
	...
	re := regexp.MustCompile(`(?P<s_year>[[:digit:]]{4})年(?P<s_month>[[:digit:]]{1,2})月(?P<s_day>[[:digit:]]{1,2})日[[:space:]]+(?P<l_day>[\p{Han}]{2,3})[[:space:]]+[\p{Han}]{3}[[:space:]]+(?P<term>[\p{Han}]*)[[:space:]]+`)
	...
	for sc.Scan() {
		...
		if len(args) == 1 {
			...
			if err = p.Db.Create(&dt).Error; err != nil {
				log.Printf(line)
				return err
			}
